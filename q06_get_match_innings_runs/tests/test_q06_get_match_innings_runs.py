...
class TestGet_match_innings_runs(TestCase):
    def test_get_match_innings_runs_return_runs(self):
        path = "./data/ipl_dataset.csv"
        ipl_df = read_csv_data_to_df(path)
        bowler = 'I Sharma'
        expected_innigs_runs = 165858
        actual_innings_runs = get_match_innings_runs().sum()
        self.assertTrue(np.all(actual_innings_runs == expected_innigs_runs),"The expected runs do not match with the value returned")
